...
class MainViewModel constructor(private val repository: MainRepository): ViewModel() {
    ...
    fun getAllPlayerofTeam(){
        ...
        repository.enqueue(object: Callback<PlayerOfTeam>{
            override fun onResponse(call: Call<PlayerOfTeam>, response: retrofit2.Response<PlayerOfTeam>) {
    ...
    fun getTopScorer(){
        ...
        repository.enqueue(object: Callback<TopScorer> {
            override fun onResponse(call: retrofit2.Call<TopScorer>, response: retrofit2.Response<TopScorer>) {
                val dataTopScorer: TopScorer? = response.body()
                if(dataTopScorer != null && dataTopScorer.data != null ){
                    playerTopScorer = dataTopScorer.data as ArrayList<com.example.standing.model.Data>
